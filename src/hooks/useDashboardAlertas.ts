  ...
  return useQuery<Alerta[]>({
    ...
    queryFn: async () => {
      ...
      (osExecucao ?? []).forEach((os) => {
        const dias = Math.floor((Date.now() - new Date(os.criado_em!).getTime()) / 86400000);
        alertas.push({ tipo: 'execucao_longa', mensagem: `OS-${os.numero} em execução há ${dias} dias`, prioridade: 2 });
      });
      ...
      (semResposta ?? []).forEach((os: any) => {
        ...
        alertas.push({ tipo: 'orcamento_pendente', mensagem: `OS-${os.numero} (${nome}) sem resposta ao orçamento há ${dias} dias`, prioridade: 2 });
      });
      ...
      (esperando ?? []).forEach((os: any) => {
        ...
        const nome = os.clientes?.nome || 'Cliente';
        alertas.push({ tipo: 'aguardando_retirada', mensagem: `OS-${os.numero} (${nome}) pronta há ${dias} dias aguardando retirada`, prioridade: 3 });
      });
