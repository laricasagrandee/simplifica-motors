...
export function useInventarioAtual() {
  return useQuery<Inventario | null>({
    ...
    queryFn: async () => {
      ...
      return data as unknown as Inventario | null;
    },
  });
}
...
export function useCriarInventario() {
  ...
  return useMutation({
    mutationFn: async () => {
      ...
      // Populate with all pecas
      const { data: pecas, error: e2 } = await supabase.from('pecas').select('id, estoque_atual');
      if (e2) throw e2;

      const itens = (pecas ?? []).map((p) => ({
        inventario_id: inventario.id,
        peca_id: p.id,
        estoque_sistema: p.estoque_atual ?? 0,
      }));

      if (itens.length > 0) {
        const { error: e3 } = await supabase.from('inventario_itens' as any).insert(itens as any);
        if (e3) throw e3;
      }
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['inventario-atual'] });
  ...
  return useMutation({
    mutationFn: async ({ id, estoque_contado, observacao }: { id: string; estoque_contado: number; observacao?: string }) => {
      const { error } = await supabase
        .from('inventario_itens' as any)
        .update({ estoque_contado, observacao: observacao || null, contado_em: new Date().toISOString() } as any)
        .eq('id', id);
      if (error) throw error;
    },
  ...
  return useMutation({
    mutationFn: async (inventarioId: string) => {
      ...
      const { data: itens, error: e1 } = await supabase
        .from('inventario_itens' as any)
        .select('peca_id, estoque_contado, diferenca')
        .eq('inventario_id', inventarioId)
      ...
      for (const item of (itens ?? []) as unknown as { peca_id: string; estoque_contado: number; diferenca: number }[]) {
        if (item.diferenca !== 0) {
          await supabase.from('pecas').update({ estoque_atual: item.estoque_contado }).eq('id', item.peca_id);
        }
      }
      ...
      const { error: e2 } = await supabase
        .from('inventarios' as any)
        .update({ status: 'finalizado', finalizado_em: new Date().toISOString() } as any)
        .eq('id', inventarioId);
      if (e2) throw e2;
    },
    onSuccess: () => {
