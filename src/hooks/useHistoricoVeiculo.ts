...
export function useHistoricoVeiculo(veiculoId: string) {
  ...
  return useQuery<OrdemServico[]>({
    ...
    queryFn: async () => {
      let q = supabase.from('ordens_servico').select('id, numero, status, problema_relatado, valor_total, criado_em, data_conclusao, funcionarios(nome)').eq('moto_id', veiculoId) as any;
