...
export function useRankingMecanicos() {
  return useQuery<MecanicoRank[]>({
    queryKey: ['dashboard-ranking-mecanicos'],
    queryFn: async () => {
      const now = new Date();
      ...
      const [{ data: osData, error: e1 }, { data: metasData }] = await Promise.all([
        supabase
          .from('ordens_servico')
          .select('mecanico_id, valor_total, funcionarios(nome)')
          .in('status', ['concluida', 'entregue'])
          .gte('criado_em', firstDay)
          .not('mecanico_id', 'is', null),
        supabase
          .from('metas_mecanico' as any)
          .select('funcionario_id, meta_os, meta_faturamento')
          .eq('mes', mes)
          .eq('ano', ano),
      ]);
      if (e1) throw e1;
      ...
      return Array.from(map.entries())
        .map(([fid, rank]) => {
          ...
          return {
            ...rank,
            meta_os: meta?.meta_os,
            meta_faturamento: meta?.meta_faturamento,
            pct_os: meta?.meta_os ? Math.round((rank.total_os / meta.meta_os) * 100) : undefined,
            pct_faturamento: meta?.meta_faturamento ? Math.round((rank.faturamento / meta.meta_faturamento) * 100) : undefined,
          };
        })
