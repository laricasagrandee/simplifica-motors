  ...
  return useMutation({
    mutationFn: async ({ pecaId, quantidade, precoCusto, observacao }: EntradaInput) => {
      ...
      const { error: movError } = await supabase.from('estoque_movimentacoes').insert({
        ...
        tipo: 'entrada',
        quantidade: qty,
        motivo: obs || 'Compra de peça',
        preco_unitario: custo,
      });
      if (movError) throw movError;

      const { data: peca, error: pecaErr } = await supabase
        .from('pecas')
      ...
      const { error: upErr } = await supabase
        .from('pecas')
        .update({
          estoque_atual: (peca.estoque_atual ?? 0) + qty,
          preco_custo: custo,
        })
        .eq('id', pecaId);
      ...
      const { error: finErr } = await supabase.from('movimentacoes').insert({
        tipo: 'saida',
        categoria: 'compra_pecas',
        descricao: `Entrada estoque: ${qty}x - ${obs || 'Compra'}`,
        valor: totalCusto,
